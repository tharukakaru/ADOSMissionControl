...
export function DgcaIndiaTemplate({ record, operator, aircraft, generatedAt }: DgcaTemplateProps) {
  return (
    <Document title={`DGCA Flight Report ${record.id}`} author={operator.operatorName ?? "ARCOS Mission Control"}>
      <Page size="A4" style={styles.page}>
        ...
        <Text style={styles.footer} fixed>
          Generated by ARCOS Mission Control · {generatedAt.toISOString()} · Retain for 5 years per DGCA audit policy
        </Text>
      </Page>
    </Document>
  );
